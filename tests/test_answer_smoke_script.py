...
def test_answer_smoke_script_skips_qdrant_for_lexical_mode(monkeypatch, tmp_path: Path) -> None:
    settings = SimpleNamespace(
        log_level="INFO",
        retrieval_min_top_score=0.25,
        qdrant_local_path=tmp_path / "qdrant",
    ...
    response = GroundedAnswerResponse(
        query="What changed in branch reports?",
        answer="Grounded lexical answer [C1].",
        is_answered=True,
        refusal_reason=None,
        citations=[
            Citation(
                ...
                source_kind="paragraph",
                score=4.2,
                text_preview="Lexical branch report evidence",
            )
        ],
    )
    trace = build_answer_trace(
        query="What changed in branch reports?",
        filters={"document_family": None, "release_label": None, "source_kind": None},
        sufficiency=sufficiency,
        answer_response=response,
        retrieval_results=[retrieved_result],
        request_id="lexical-answer-smoke-test-request",
        retrieval_metadata={"retrieval_mode": "lexical"},
    )
    trace_output_path = tmp_path / "exports" / "answer_runs" / "lexical-answer-smoke-test-request.json"
    captured: dict[str, object] = {}

    def fail_if_qdrant_created(path):
        raise AssertionError("Lexical answer smoke should not create a Qdrant client")
    ...
    monkeypatch.setattr(run_answer_smoke_test, "get_settings", lambda: settings)
    monkeypatch.setattr(run_answer_smoke_test, "build_retrieval_runtime_config", lambda loaded_settings: retrieval_config)
    monkeypatch.setattr(run_answer_smoke_test, "create_persistent_qdrant_client", fail_if_qdrant_created)
    monkeypatch.setattr(run_answer_smoke_test, "run_grounded_answer_query", fake_run_grounded_answer_query)
    monkeypatch.setattr(
        sys,
        "argv",
