...
def test_full_claim_review_packet_includes_structural_pass(tmp_path: Path) -> None:
    report_path = tmp_path / "report.json"
    eval_path = tmp_path / "cases.jsonl"
    trace_directory = tmp_path / "run-1" / "answer_traces"
    trace_directory.mkdir(parents=True)
    ...
    entries = export_fdd_manual_review_packet.load_review_entries(
        report_path,
        eval_path,
        include_passed_claim_reviews=True,
    )
    output_path = export_fdd_manual_review_packet.write_manual_review_packet(
        entries,
        tmp_path / "packet.md",
    ...
    assert len(entries) == 1
    packet = output_path.read_text(encoding="utf-8")
    assert "every generated answer requiring semantic SME review" in packet
    assert "`accepted`" in packet
